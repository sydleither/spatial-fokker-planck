...
def spsb_plot(transform, data_type, source, subsample_length, time, desired_sm):
    ...
    for sample in os.listdir(data_path):
        if not os.path.isdir(f"{data_path}/{sample}"):
            continue
        s_coords, r_coords, config = read_sample(data_path, sample, time)
    ...
    file_name = f"sm={desired_sm}_sub={subsample_length}_time={time}.png"
    fig.savefig(f"{save_loc}/{file_name}", bbox_inches="tight")
...
def main():
    ...
    if data_type == "self":
        ...
        spsb_plot(args.transform, data_type, args.source, args.subsample_length, args.time, args.sm)
