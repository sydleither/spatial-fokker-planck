...
def main():
    ...
    save_info = "_".join([str(x) for x in [args.transform, n, mu, c]])
    game_order = [x for x in game_colors if x != "Unknown"]

    # Plot curves
    plot_curves(save_loc, save_info, game_order, supports, spsb_pdfs, fp_pdfs, games)
    ...
    #confusion_matrix(save_loc, save_info, df_mat)
